...
└── examples/                     # 书源示例文件
    └── sourceModelList.xbs       # 1463 个书源的合集（见下方说明）
```
...
`examples/sourceModelList.xbs` 是一个包含 **1463 个书源**的合集文件（约 12MB），可直接用于香色闺阁 App 导入。

> 该文件体积较大，未包含在 Git 仓库中，而是作为 **GitHub Release 附件**提供。
> 下载链接：[v1.0.0 Release](https://github.com/Awswbd888/book-source-tools/releases/tag/v1.0.0)

**使用方式：**
```bash
# 下载后解密为 JSON 查看
python scripts/booksource_tool.py decrypt examples/sourceModelList.xbs sourceModelList.json

# 查看书源列表
python scripts/booksource_tool.py inspect examples/sourceModelList.xbs --list

# 合并新书源到此合集
python scripts/booksource_tool.py merge new_source.xbs --into examples/sourceModelList.xbs
```
